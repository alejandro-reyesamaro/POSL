...
{
    create_index(indexes, 0, max, dim);
}
...
NIndex * RandIndexGenerator::create_index(int initial_value, int max_value, int dim)
{
    if(dim != 1)
    {
        ...
        for(int i = initial_value + 1; i <= max_value; i++)
        {
...
void RandIndexGenerator::create_index(vector<NIndex *> & indexes, int initial_value, int max_value, int dim)
{
    for(int i = initial_value; i<= max_value; i++)
    {
        if(i + dim - 1 <= max_value)
        {
            ...
            if(dim > 1)
                create_index(ni->nexts, i+1, max_value, dim - 1);
        }
    }
}
...
{
    // PELIGRO!!! si c > n! / [ (n - k)! x k! ] explota
