#include "one_sorted_change_companion_neighborhood.h"
...
OneSortedChangeCompanionNeighborhood::OneSortedChangeCompanionNeighborhood(int _config_size)
    : Neighborhood(_config_size),
      changeAtBhv(std::make_shared<SortedApplyChangeBehavior>(_config_size)),
      changes(0),
      indexes(Tools::generateMonotony(1, _config_size - 2)),
      studied_indexes (max(2, (_config_size-2)/3))
{}

std::shared_ptr<POSL_Iterator> OneSortedChangeCompanionNeighborhood::getIterator()
{
    return std::make_shared<ElementsChangeIterator>(shared_from_this());
}

void OneSortedChangeCompanionNeighborhood::Init(shared_ptr<PSP> psp, std::vector<int> & _configuration)
{
...
void OneSortedChangeCompanionNeighborhood::updateChanges(shared_ptr<Randomizer> rand)
{
    changes.clear();
    //int sz = changes.size();
    ...
    for(unsigned int i = 0; i < studied_indexes; i++)
    {
...
bool OneSortedChangeCompanionNeighborhood::is_valid_distance(int new_index, int new_value)
{
...
std::shared_ptr<FactoryPacker> OneSortedChangeCompanionNeighborhood::BuildPacker(){ throw "Not implemented yet"; }

std::vector<int> OneSortedChangeCompanionNeighborhood::neighborAt(int index)
{
