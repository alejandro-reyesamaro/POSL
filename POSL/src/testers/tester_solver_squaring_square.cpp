...
string Tester_SolverSquaringSquare::test()
{
    ...
    SquaringSquare * ss = new SquaringSquare(10,squares);
    Benchmark * bench = ss;
    //vector<int> squares({2, 3, 4, 6, 7, 8, 12, 13, 14, 15, 16, 17, 18, 21, 22, 23, 24, 26, 27, 28, 50, 60});
    //SquaringSquare * bench = new SquaringSquare(110, squares);
    //vector<int> squares({1, 1, 1, 1, 2, 2, 4, 4, 4, 4, 4, 4, 4, 6, 6, 10, 10});
    //SquaringSquare * bench = new SquaringSquare(20, squares);
    Solution * sol = new Solution(bench->Domains());
    bench->UpdateSolution(sol);

    PSP * psp = new PSP(ARGC, ARGV, bench);

    CreateSolverStrategy * css = new ForSquaringSquareCSS(ss);
