...
string Tester_CostOfSolutionGolfers::test()
{
    ...
    shared_ptr<Solution> sol442 = make_shared<Solution>(psp442->GetBenchmark()->Variable_Domain(), Tester::Golfers_442_c0());
    int c1 = psp442->GetBenchmark()->solutionCost(sol442);

    sol442 = make_shared<Solution>(psp442->GetBenchmark()->Variable_Domain(), Tester::Golfers_442_c4());
    int c2 = psp442->GetBenchmark()->solutionCost(sol442);

    shared_ptr<Benchmark> bench553(make_shared<Golfers>(5,5,4));
    shared_ptr<Solution> sol553(make_shared<Solution>(bench553->Variable_Domain(), Tester::Golfers_554_c0()));
    //bench->UpdateSolution(sol);
    shared_ptr<PSP> psp553(make_shared<PSP>(bench553));
    int c3 = psp553->GetBenchmark()->solutionCost(sol553);

    shared_ptr<Benchmark> bench662(make_shared<Golfers>(6,6,2));
    shared_ptr<Solution> sol662(make_shared<Solution>(bench662->Variable_Domain(), Tester::Golfers_662_c0()));
    shared_ptr<PSP> psp662(make_shared<PSP>(bench662));
