...
string Tester_CostOfSolutionCostasArray::test()
{
    ...
    shared_ptr<Solution> solita = make_shared<Solution>(pspito->GetBenchmark()->Variable_Domain(), Tester::Costas_6_c0());
    int co1 = pspito->GetBenchmark()->solutionCost(solita);
    cout << co1 << endl;

    shared_ptr<Solution> sol = make_shared<Solution>(psp->GetBenchmark()->Variable_Domain(), Tester::Costas_18_c0());
    int co2 = psp->GetBenchmark()->solutionCost(sol);
    cout << co2 << endl;

    shared_ptr<Solution> sol16 = make_shared<Solution>(psp->GetBenchmark()->Variable_Domain(), Tester::Costas_16_c0());
    int co3 = psp16->GetBenchmark()->solutionCost(sol16);
    cout << co3 << endl;

    //shared_ptr<Solution> sol = make_shared<Solution>(psp->GetBenchmark()->Variable_Domain(), c3);
