...
string Tester_Solver_GolombRuler::test()
{
    ...
    // cm1 |-> [ cm21 (p) cm22 ] :
    Operator * O_sec_1_Grho1 = new SequentialExecOperator(cm11, cm22);//G_rho1);
    // cm1 |-> cm2 :
    ...
    Operator * O_cyc1 = new CyclicOperator(G_sec41, new LoopBoundExpression(200));
    ...
    // solver with NO restart
    //Solution * final_solution = (Solution *)G_sec3->execute(psp, new Seed());

    cout << bench->ShowSolution(final_solution) << endl;
    ...
    cout << bench->ShowSolution(best_solution) << endl;
    int best_cost = psp->GetBenchmark()->solutionCost(best_solution);
    cout << "Best cost: "<< best_cost << endl;

    cout << endl;

    cout << "Iterations: " << psp->GetIterations() << endl;
    cout << "Time (milisecs): " << psp->GetTime() << endl;
    return "SOLVER: OK !";
}
