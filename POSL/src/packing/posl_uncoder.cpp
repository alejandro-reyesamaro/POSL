...
vector<shared_ptr<POSL_Solver>> PoslUncoder::uncode_declarations(string code, std::shared_ptr<Benchmark> bench)
{    
    HashMap<string, string> strategies;
    SolverDeclarationUncoder sd_unc;
    vector<shared_ptr<POSL_Solver>> solvers_list;
    ...
    while(code.empty())
    {
        ...
        if(type_declar == SOLVER_KEYWORD)
        {
            ...
            if(strategies.existsKey(strategy_name))
            {
                ...
                st_instance = make_shared<ComputationStrategy>(strategy_code,
                                                               sdec.Operation_Module_Instance_Names,
                                                               sdec.Open_Channel_Instance_Names,
                                                               bench);
                solver = make_shared<POSL_Solver>(bench, st_instance);
                solvers_list.push_back(solver);
            }
            else
